"""Minimal EMA loss-normalizer extracted from the Lumina training stack.

The full training losses are intentionally NOT shipped in this inference package.
``LuminaModel`` constructs
``self.loss_ema = LossEMANormalizer(LUMINA_LOSS_NAMES, per_loss_gating_enabled=...)``
in its ``__init__`` (see ``lumina/models/model.py``), which registers one ``scale_<name>``,
one ``init_<name>`` (bool), and one ``init_scale_<name>`` buffer per loss name. Those buffers
are part of every trained checkpoint's state dict, so reproducing them here verbatim — same
module (``lumina.models.losses.LossEMANormalizer``), same names, same order — is what lets a
checkpoint load with ``strict=True`` without shipping the training loss machinery.

``update_and_normalize`` is a training-only method and is never called at inference; it is kept
verbatim from the training source (revision ``f2f7f9d``) for fidelity. This module imports only
``torch`` — none of the training loss code.
"""
...
# Canonical Lumina loss names. Order matters only for buffer registration / checkpoint compatibility;
# ``LossEMANormalizer`` registers one buffer group per name from this tuple, in this order.
LUMINA_LOSS_NAMES: tuple[str, ...] = (
    "mlm",
    "conservation",
    "splice",
    "region",
...
class LossEMANormalizer(nn.Module):
    ...
    def __init__(
        self,
        names: tuple[str, ...] = LUMINA_LOSS_NAMES,
        *,
        decay: float = 0.99,
        eps: float = 1e-8,
        ...
        # collapses toward 0 (e.g. rc_consistency once the embedding is RC-invariant) drives its EMA
        # scale toward 0, and dividing the raw loss by ~0 amplifies that loss's gradient unboundedly
        # (observed: rc scale 2e-4 → ~4000x amplification → exploding grad-norm → grad-clip discards
        # the real objectives' gradients → no learning). Flooring caps amplification at 1/min_scale
        # and lets a satisfied loss fade to a small contribution instead of being re-inflated to ~1.
        self.min_scale = float(min_scale)
        # T4-8b — per-loss gating. The min_scale floor still AMPLIFIES any loss whose natural scale is
        # below 1.0: normalized = raw/scale, and a satisfied loss whose scale falls to the 0.25 floor is
        ...
        # Default OFF ⇒ the denominator branch is byte-identical to the pre-T4-8b behavior. Matters more
        # in current: more objectives ⇒ more satisfied losses the floor would otherwise mis-amplify.
        ...
        *,
        ...
        for name, raw in raw_losses.items():
            if name in self.names:
                # Validation consumes the frozen training scale and never initializes or advances buffers.
                if self.training and float(valid_counts.get(name, 0.0)) >= float(min_valid.get(name, 1)):
                    self._update_scale(name, raw)
                scale = getattr(self, f"scale_{name}")
                initialized = getattr(self, f"init_{name}")
                frozen_scale = torch.where(initialized, scale.detach(), torch.ones_like(scale))
                denom = frozen_scale.to(raw.device).clamp_min(self.min_scale)
                ...
                scale_tensors[name] = frozen_scale.detach().reshape(()).float()
            else:
                normalized[name] = raw
                scales[name] = 1.0
        ...
        if scale_tensors:
            ...
            for key, val in zip(keys, torch.stack([scale_tensors[k] for k in keys]).tolist(), strict=False):
                scales[key] = val
