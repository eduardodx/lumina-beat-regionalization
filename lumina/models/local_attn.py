...
class LocalWindowAttention(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, window: int, dropout: float, qk_norm: bool = False) -> None:
        super().__init__()
        ...
        # qk-norm (shakeout S3 / gnorm hypothesis H3): L2-normalize q,k before the dot product so the
        # attention logits cannot grow unboundedly. A learnable per-head temperature restores a usable
        # logit scale (init head_dim so the post-1/sqrt(d) logit ~ sqrt(head_dim)*cosine, matching standard
        # attention's range). The parameter is only allocated when enabled, so a qk_norm=False module is
        # bit-identical and state-dict-compatible with the pre-qk-norm model.
        self.qk_norm = bool(qk_norm)
        self.qk_scale: nn.Parameter | None = None
        if self.qk_norm:
            self.qk_scale = nn.Parameter(torch.full((self.n_heads,), float(self.head_dim)))
    ...
    def _reshape_qkv(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.qk_norm and self.qk_scale is not None:
            # Bound the logits at the source (both the flash and chunked-SDPA paths consume q,k and apply
            # the usual 1/sqrt(head_dim) scaling, so the learnable temperature lands consistently).
            q = F.normalize(q, dim=-1) * self.qk_scale.view(1, self.n_heads, 1, 1)
    ...
    def _flash_forward(
        ...
        except Exception as exc:  # windowed flash kernel unsupported here - disable and fall back to SDPA
            if self._flash_attn_supports_window:
                print(f"flash_attn_window=unsupported detail={exc}")
            self._flash_attn_supports_window = False
            return None
