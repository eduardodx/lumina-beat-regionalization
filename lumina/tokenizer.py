"""DNA string → token-id encoding for Lumina inference.

Lumina uses a fixed character-level vocabulary (no learned tokenizer): ``A/C/G/T/N`` map to the
training ids, ``PAD``/``MASK``/``UNK`` are the special tokens (see ``lumina.constants``). These helpers
mirror the training encode path (``lumina.data.genome.encode_dna``) and add batching/padding + a
reverse-complement helper for the model's RC-augmentation use.
"""
...
# Inverse vocab for decoding. DNA_VOCAB only holds A/C/G/T/N; add the special tokens so a full
# id→symbol map is available. (constants.py is vendored verbatim from training and has no ID_TO_DNA.)
ID_TO_DNA: dict[int, str] = {token_id: base for base, token_id in DNA_VOCAB.items()}
ID_TO_DNA[PAD_ID] = "<pad>"
ID_TO_DNA[6] = "<mask>"  # MASK_ID
ID_TO_DNA[UNK_ID] = "<unk>"
...
def encode_dna_sequence(
    ...
    *,
    ...
    """Encode a DNA string into Lumina token ids.

    A, C, G, T, and N use the training vocabulary. Other IUPAC ambiguity codes default to N
    (``unknown_policy="n"``) so real genomic windows pass through inference unchanged; set
    ``"unk"`` to route them to the dedicated UNK id, or ``"error"`` to reject them.
    """
...
def batch_encode_dna(
    ...
    *,
    ...
    """Encode and pad DNA strings into ``{"input_ids", "attention_mask"}`` for Lumina inference."""
...
def decode_dna_ids(token_ids: Sequence[int] | torch.Tensor, *, skip_special: bool = True) -> str:
    """Decode token ids back to a compact DNA-ish string (special tokens dropped by default)."""
