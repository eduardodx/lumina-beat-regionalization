"""Load a Lumina model from a native training checkpoint (local path or ``s3://``).

The inference package is weights-agnostic: it ships no ``.pt`` files. Point
``load_model_from_checkpoint`` at a local file or an ``s3://`` URI (e.g. the R03 release
``s3://croma-bioai-lumina-artifacts-us-east-2/experiments/LUM-20260719-001/runs/R03/checkpoints/final/best_checkpoint.pt``)
and it resolves the architecture from the checkpoint's own ``config["model_config"]``, rebuilds
``LuminaModel``, and loads the weights with ``strict=True``.

Native ``best_checkpoint.pt`` / ``final_checkpoint.pt`` files store the state dict under ``model``
and the architecture under ``config["model_config"]``; they load with ``strict=True`` (the vendored
module tree — including ``loss_ema.*`` buffers — matches the training source revision ``f2f7f9d``).
The EMA export ``final_ema.pt`` omits the optimizer / ``loss_ema`` buffers and may carry an empty
config; pass ``strict=False`` for it (the checkpoint model_config, or the packaged defaults, supply
the architecture).
"""
...
from lumina.models.model import LuminaConfig, LuminaModel

# fields() returns the full LuminaConfig field set (LuminaBackboneConfig base + Lumina-only fields),
# i.e. every key present in a trained ``model_config``.
_CONFIG_FIELDS = {field.name for field in fields(LuminaConfig)}
...
def normalize_config_dict(
    ...
    *,
    ...
    """Canonicalize an override dict onto the LuminaConfig defaults.

    Keys are hyphen-normalized (``-`` → ``_``); duplicates raise. Unknown keys raise unless
    ``ignore_unknown`` is set, in which case they are dropped (used for checkpoint model_config,
    which may carry derived/legacy keys the current dataclass no longer declares).
    """
    ...
    if unknown and not ignore_unknown:
        raise ValueError(f"Unknown Lumina config keys in {source}: {', '.join(unknown)}")
    for key in unknown:
        canonical.pop(key)
...
def config_from_checkpoint(
    ...
    *,
    ...
    if isinstance(model_name, str) and model_name.strip().lower() not in _MODEL_NAMES:
        raise ValueError(
            f"Checkpoint declares model={model_name!r}, expected one of {sorted(_MODEL_NAMES)} (Lumina)."
        )
    ...
    # ignore_unknown: the checkpoint's model_config is trusted (it was serialized by the pinned source
    # revision); dropping any key the current dataclass doesn't declare keeps loading robust.
    data = normalize_config_dict(raw_model_config, source="checkpoint model_config", ignore_unknown=True)
    if config_overrides is not None:
        data = normalize_config_dict({**data, **dict(config_overrides)}, source="config_overrides")
    return LuminaConfig(**data)


def _torch_load_local_checkpoint(path: str | Path, map_location: str | torch.device = "cpu") -> dict[str, Any]:
    # weights_only=False: Lumina checkpoints carry non-tensor metadata (config, step, rng, ...).
    try:
...
def torch_load_checkpoint(
    ...
    *,
    ...
    For ``s3://bucket/key`` paths, the object is downloaded to a temporary file first so PyTorch can
    load it through its normal checkpoint reader.
    """

    if is_s3_uri(path):
        parsed = urlparse(os.fspath(path))
        filename = Path(parsed.path).name or "checkpoint.pt"
        with tempfile.TemporaryDirectory(prefix="lumina-checkpoint-") as tmp_dir:
...
def strip_state_dict_prefixes(state_dict: Mapping[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    """Strip DDP (``module.``) and ``torch.compile`` (``_orig_mod.``) prefixes so a raw ``LuminaModel``
    accepts the training state dict. Nested prefixes are stripped iteratively."""
...
def build_model(config_overrides: Mapping[str, Any] | None = None) -> LuminaModel:
    """Build a fresh (untrained) LuminaModel from an override dict onto the packaged defaults."""

    cfg = LuminaConfig(**normalize_config_dict(config_overrides, source="config_overrides"))
...
def load_model_from_checkpoint(
    ...
    *,
    ...
    The state dict is read from ``model`` / ``model_state_dict`` / ``state_dict`` (DDP / compile
    prefixes stripped) and the architecture from ``config["model_config"]``. Returns the model in
    ``eval()`` mode. Use ``strict=False`` only for the EMA export ``final_ema.pt``.
    """
