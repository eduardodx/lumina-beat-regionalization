#!/usr/bin/env bash
# Dispatch the beat-v10 assay-rescue hybrid head.
#
# Hypothesis: profile/count BioAux is globally strong, but chromatin-like
# tracks can benefit from a small local/BioAux rescue branch with learned
# per-track gates. The branch starts weakly active so the run tests rescue
# capacity without immediately overriding the validated profile/count path.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export EXPERIMENT="${EXPERIMENT:-beat-v10-24k-v10-assay-rescue-hybrid-rc005-longtail-lr125-llrd085-seed0}"
export MODEL_NAME="${MODEL_NAME:-Lumina beat-v10 24k AssayRescue Hybrid RC0.05 LongTail LR1.25e-4 LLRD0.85}"
export FUNCTIONAL_HEAD_TYPE="${FUNCTIONAL_HEAD_TYPE:-v10-assay-rescue-hybrid}"
