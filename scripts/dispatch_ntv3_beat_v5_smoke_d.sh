#!/usr/bin/env bash
# Dispatch the fourth diagnostic NTv3 smoke run: semantically identical to smoke.sh but
# with early-step and first-N-item diagnostics enabled so we can phase-localize the
# step-2+ failure that r5 left open.
#
...
EXPERIMENT="${EXPERIMENT:-ntv3-beat-v5-smoke-d-seed42}"
...
export LUMINA_NTV3_DEBUG_DATASET_ITEMS="${LUMINA_NTV3_DEBUG_DATASET_ITEMS:-3}"
