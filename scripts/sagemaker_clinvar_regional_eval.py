#!/usr/bin/env python3
"""Submit regional ClinVar evaluation of a fine-tuned model to SageMaker."""
...
JOB_ENTRYPOINT = Path("scripts/clinvar_regional_eval_job.py")
...
DEFAULT_BASE_CHECKPOINT_S3_PREFIX = "s3://ai4bio-lumina/releases/lumina-beat-v10-20260527182934/"
DEFAULT_FINETUNED_S3_PREFIX = (
...
def parse_args(argv: list[str] | None = None) -> tuple[argparse.Namespace, list[str]]:
    launcher_args, eval_args = split_cli_args(sys.argv[1:] if argv is None else argv)
    ...
    parser.add_argument("--max-run-hours", type=int, default=8)
    ...
    parser.add_argument("--base-checkpoint-s3-prefix", default=DEFAULT_BASE_CHECKPOINT_S3_PREFIX)
    parser.add_argument("--finetuned-s3-prefix", default=DEFAULT_FINETUNED_S3_PREFIX)
    parser.add_argument("--detach", action="store_true")
    return parser.parse_args(launcher_args), eval_args
...
def build_job_name(experiment: str) -> str:
    ...
    for redundant_prefix in ("clinvar-regional-eval-", "regional-eval-", "eval-"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    s3_output = f"s3://{args.bucket}/lumina-ssm/clinvar-regional-eval/{experiment}/sagemaker-artifacts/"
    max_wait_hours = args.max_wait_hours if args.max_wait_hours is not None else args.max_run_hours * 2

    print(f"Creating SageMaker ClinVar regional eval job: {args.experiment}")
    ...
    print(f"  Base checkpoint channel: {args.base_checkpoint_s3_prefix}")
    print(f"  Fine-tuned channel: {args.finetuned_s3_prefix}")
    print(f"  Output artifacts: {s3_output}")
    print(f"  Eval args: {' '.join(eval_args) if eval_args else '(defaults)'}")
    ...
    try:
        ...
        training_job = trainer.train(
            input_data_config=[
                ...
                InputData(channel_name="base", data_source=args.base_checkpoint_s3_prefix),
                InputData(channel_name="finetuned", data_source=args.finetuned_s3_prefix),
            ],
